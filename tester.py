...
with open("raport_100k.txt", "w") as report_file:
    ...
    report_file.write(f"hit to faults ratio for opt {(sum(hit_list_opt) / sum(faults_list_opt))*100}\n")
    report_file.write(f"average faults for lru {sum(faults_list_lru) / len(faults_list_lru)}\n")
    report_file.write(f"hit to faults ratio for lru {(sum(hit_list_lru) / sum(faults_list_lru))*100}\n")
    report_file.write(f"average faults for fifo {sum(faults_list_fifo) / len(faults_list_fifo)}\n")
    report_file.write(f"hit to faults ratio for fifo {(sum(hit_list_fifo) / sum(faults_list_fifo))*100}\n")
    ...
    report_file.write(f"hit to faults ratio for random replacment {(sum(hit_list_random_replacment) / sum(faults_list_random_replacment))*100}\n")
